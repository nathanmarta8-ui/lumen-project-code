...
(function () {
  ...
  function newsletterTileHTML() {
    return '<div class="tile-news reveal" id="newsletter">' +
      ...
      '<p>Tuesday and Friday. No noise. No hype. Every story pre-cleared by the Lumen Standard.</p>' +
      newsletterFormHTML() +
  ...
  function renderHome() {
    ...
    /* positioning proof statement, directly under the nav, above the hero.
       KEEP IN SYNC with build.js homepage SSR so it does not flash on hydrate. */
    ...
    html += '<div class="container board">';
    /* static lead-in where the old ticker sat */
    html += '<p class="board-leadin">Every claim verified against the Lumen Standard. Latest stories below.</p>';
      ...
      function miniList(title, slug, items) {
        return '<div class="reveal"><div class="section-head"><h2 class="section-title" style="font-size:24px">' + title + '</h2>' +
          ...
          '<div class="row-list">' + (items.length ? items.map(function (s) {
            return '<div class="row"><div><span class="cat-label">' + esc(s.subCategory || s.category) + '</span>' +
              '<h3 style="font-size:18px"><a href="' + storyUrl(s) + '">' + esc(plainHead(s.headline)) + '</a></h3></div></div>';
