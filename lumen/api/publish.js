...
export default async function handler(req, res) {
  ...
  const detailOf = async (r) => { try { return await r.json(); } catch (e) { return { status: r.status }; } };

  try {
    /* 1. base branch head sha */
    let r = await gh('/git/ref/heads/' + encodeURIComponent(BASE_BRANCH));
    if (!r.ok) return res.status(502).json({ error: 'Cannot read base branch "' + BASE_BRANCH + '". Check GITHUB_REPO and token scope.', detail: await detailOf(r) });
    ...
    if (!r.ok) return res.status(502).json({ error: 'Could not create branch ' + branch, detail: await detailOf(r) });
    ...
    if (!r.ok) return res.status(502).json({ error: 'Could not commit data/stories.js', detail: await detailOf(r) });
    ...
    if (!r.ok) return res.status(502).json({ error: 'Committed to ' + branch + ', but could not open the PR. Open it manually on GitHub.', detail: await detailOf(r) });
