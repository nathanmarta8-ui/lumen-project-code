...
export default async function handler(req, res) {
  ...
  /* The site lives in the lumen/ subfolder (Vercel Root Directory = lumen), so
     the real source file is lumen/data/stories.js, NOT data/stories.js at the
     repo root, which the build never reads. All GitHub paths use this. */
  const STORIES_PATH = 'lumen/data/stories.js';
  ...
  stories.forEach((s, i) => {
    ...
    /* optional verification provenance: allowed through, must be string or null */
  ...
  try {
    ...
    let fileSha;
    r = await gh('/contents/' + STORIES_PATH + '?ref=' + encodeURIComponent(BASE_BRANCH));
    if (r.ok) { fileSha = (await r.json()).sha; }

    /* 4. commit the new lumen/data/stories.js onto the publish branch */
    ...
    r = await gh('/contents/' + STORIES_PATH, { method: 'PUT', body: JSON.stringify(put) });
    if (!r.ok) { const e = await ghErr(r); return res.status(502).json({ error: 'Could not commit ' + STORIES_PATH + ' (' + e.status + '): ' + e.message }); }
    ...
    r = await gh('/pulls', {
      ...
      body: JSON.stringify({
        ...
        body: 'Automated publish from the Lumen admin.\n\n' +
          '- ' + stories.length + ' stories in `' + STORIES_PATH + '`\n' +
