...
function listingHTML(list) {
  ...
  return '<div class="ssr-listing">' + list.map(s => {
    ...
    const tag = [s.sourceType || 'Peer-reviewed', s.journal, s.patientN ? 'n=' + s.patientN : '', 'Impact ' + esc(s.impact) + '/10']
...
function buildHomepage(stories) {
  ...
  const ssr = '<section class="ssr-home">' +
    ...
    '<p class="ssr-intro">Genuine medical and scientific breakthroughs, translated into clear, honest stories. Every story passes a five-point evidence filter.</p>' +
...
function categoryHTML(catName, slug, stories) {
  ...
  const desc = 'The latest evidence-first ' + catName + ' breakthroughs on Lumen, each graded against a five-point editorial standard.';
...
function build() {
  ...
  console.log(`[lumen build] pre-rendered ${n} story pages, ${catCount} category pages, homepage ${homeOk ? 'baked' : 'SKIPPED'}, sitemap + llms.txt`);
}
build();
